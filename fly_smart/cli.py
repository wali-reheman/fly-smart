...
# Resolve script path: pip install puts it next to this file; source dir is two levels up
HERE = Path(__file__).parent.resolve()

# Installed package: fly_smart/flight_transfer_finder.py
INSTALLED_SCRIPT = HERE / "flight_transfer_finder.py"

# Source/development: fly-smart/references/flight_transfer_finder.py
SOURCE_SCRIPT = HERE.parent / "fly-smart" / "references" / "flight_transfer_finder.py"

# Hermes scripts dir fallback
HERMES_SCRIPT = Path.home() / ".hermes" / "scripts" / "flight-transfer-finder.py"

if INSTALLED_SCRIPT.exists():
    SCRIPT_PATH = INSTALLED_SCRIPT
elif SOURCE_SCRIPT.exists():
    SCRIPT_PATH = SOURCE_SCRIPT
elif HERMES_SCRIPT.exists():
    SCRIPT_PATH = HERMES_SCRIPT
else:
    SCRIPT_PATH = INSTALLED_SCRIPT  # fail with clear error downstream
